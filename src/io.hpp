...
void IO::binary_file_read(const std::string inputFile, std::vector<std::vector<struct Tile<Weight>>>& tiles, const uint32_t tile_height, const uint32_t tile_width) {
   ...
   // fin.seekg(start_offset, std::ios_base::beg);
    
    
    
    //printf("Rank=%d share=%lu start_offset=%lu end_offset=%lu\n", Env::rank, share, start_offset, end_offset);
    ...
    #pragma omp parallel
    {
        ...
        printf("Rank=%d:%d share=%lu start_offset=%lu end_offset=%lu %lu\n", Env::rank, tid, share_t, start_offset_t, end_offset_t, start_offset_t%sizeof(struct Triple<Weight>));
